...
\begin{tabular}{P{4em} P{6em} P{8em} P{8em} P{10em}} 
  \rowcolor{Blue}
    ...
    \rowcolor{LighterBlue}
    0.7.4 & 30/12/2023 & Giovanni Moretti & Giulia Dentone & Update della sezione di codifica \\
    \rowcolor{LightBlue}
...
\begin{itemize}
    ...
    \item \custombold{Indirizzato a:} Indica gli attori a cui è indirizzato il requisito.
    \item \custombold{Casi d'uso collegati:} Indica i codici di eventuali casi d'uso collegati al requisito.
    \item \custombold{Fonte:} Indica l'origine del requisito.
\end{itemize}
\subsubsection*{Casi d'uso}
I casi d'uso rappresentano un comportamento o un modo di utilizzare il prodotto e vengono descritti graficamente attraverso l'uso di diagrammi UML\textsubscript{G}.
...
Ogni caso d'uso è classificato attraverso l'utilizzo di un codice identificativo. Il gruppo ha definito uno standard per questo codice, che sarà spiegato di seguito.\\
\custombold{Codice Identificativo}\\
\begin{center}
...
\begin{itemize}
    ...
    \item \custombold{Numero caso d'uso:} Numero associato al caso d'uso principale;
...
\begin{itemize}
    \item \custombold{Node.js}: è un runtime system\textsubscript{G} open source\textsubscript{G} multipiattaforma orientato agli eventi per l'esecuzione di codice JavaScript\textsubscript{G};\\
    \begin{center}
    ...
    \end{center}
    \item \custombold{MongoDB Atlas}: è il servizio di database cloud completamente gestito offerto da MongoDB, Inc. È progettato per semplificare la gestione, l'implementazione e la scalabilità dei database MongoDB su AWS, Azure e Google Cloud Platform;\\
    \begin{center}
        \href{https://www.mongodb.com/atlas/database}{www.mongodb.com/atlas/database}
    \end{center}
    \item \custombold{Amazon AWS}:  è una piattaforma di servizi cloud che offre una vasta gamma di servizi informatici affidabili, scalabili ed economici. Questi servizi consentono alle imprese di eseguire applicazioni e gestire risorse IT senza dover investire in infrastrutture hardware costose e complesse;\\
    \begin{center}
        \href{https://aws.amazon.com/it/}{aws.amazon.com/it/}
    \end{center}
    \begin{itemize}
        \item \custombold{AWS API Gateway}: è un servizio gestito che semplifica la creazione, la pubblicazione, la manutenzione, il monitoraggio e la sicurezza di API (Application Programming Interface) per le applicazioni su AWS. Consente agli sviluppatori di creare API RESTful o WebSocket che consentono alle applicazioni di comunicare tra loro o con servizi back-end;\\
        \begin{center}
...
Gli incontri vengono organizzati almeno una volta alla settimana e vengono fatti da remoto su google meet. Servono per capire come il gruppo stà procedendo e per evitare la modalità sottomarino, ovvero il rischio che ognuno si prenda le cose da fare e non avvisi degli avanzamenti delle task. Sempre con questo scopo, ogni due giorni viene effettuato un daily meeting di cui non si rende il verbale. Alla fine di ogni incontro settimanale, di cui si è steso il verbale, si decide il giorno e l'ora del prossimo meeting. Per farlo sono stati stabiliti due metodi: \\
Il primo metodo: il responsabile propone un giorno e se tutti sono disponibili, si sceglie quella data altrimenti si va avanti a proporre.\\
Il secondo metodo: si utilizza un sondaggio fatto al momento, sempre dal responsabile, tramite telegram ed il giorno più votato verrà scelto per l'incontro. \\
Appena deciso il giorno il responsabile scrive nel calendario su Google il giorno e l'ora dell'incontro, così facendo ogni membro del gruppo avrà nel calendario un promemoria della riunione.\\
Se durante una runione interna si riscontra di non essere in grado a rispondere a dei quesiti, decidiamo di contattare l'azienda tramite Slack. Solitamente il gruppo propone un incontro e l'azienda ci comuninca i giorni in cui sono disponibili e tramite un sondaggio decidiamo il giorno migliore per tutti. Gli incontri formativi vengono effettuati in presenza, mentre quelli informativi da remoto su Google Meet. 


\subsubsection{Gestione della comunicazione interna ed esterna}
La comunicazione interna avviene tramite Telegram. Viene usato per chiarire dubbi e per tenersi aggiornati sulle cose che sono state fatte.\\
Per la comunicazione esterna abbiamo un canale slack, di cui fanno parte anche i responsabili dell'azienda proponente. Il canale viene utilizzato solo su domande veloci, ad esempio per chiedere degli incontri. Se ci sono domande più complesse, che hanno bisogno di più tempo per le risposte, viene inviata una mail, come suggerito dall'azienda.\\
Il responsabile è l'unico che scrive i messaggi per la comunicazione esterna. 

\subsubsection{Gestione delle task}
Le task vengono gestite tramite GitHub. Il responsabile, alla fine di ogni pianificazione dello Sprint aggiunge nella colonna "to-do" di GitHub le attività che si erano concordate. Ogni membro del gruppo quando inizia a svolgere un'attività la sposta nella colonna "in progress", e quando viene terminata crea il commit in cui chiude la task e automaticamente viene spostata in "done".  
...
\begin{itemize}
    ...
    \item \custombold{Revisione dei requisiti e del design}:  Prima di procedere con lo sviluppo effettivo del software, vengono esaminati e validati i requisiti e il design\textsubscript{G} del sistema. Si verifica che i requisiti siano completi, corretti e comprensibili, e che il design sia in linea con gli obiettivi e le specifiche del progetto.
    ...
    \item \custombold{Gestione della configurazione}: Durante tutto il processo di verifica, è essenziale gestire la configurazione del software e dei suoi componenti per assicurare la tracciabilità e la riproducibilità degli ambienti di test e dei risultati ottenuti.
\end{itemize}
...
\begin{itemize}
    ...
    \item Prima di aggiungere una parola nel Glossario ogni membro può velocemente verificare se la parola è già presente al suo interno tramite la legenda.
\end{itemize}  
